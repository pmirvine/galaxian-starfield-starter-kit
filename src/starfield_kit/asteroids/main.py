...
SHIP_TURN = 230  # degrees per second
SHIP_THRUST = 300  # px/s^2 while holding UP
SHIP_DRAG = 0.3  # gentle speed bleed (the arcade had none; this is kinder)
SHIP_MAX_SPEED = 460
BULLET_SPEED = 540
BULLET_LIFE = 1.1  # seconds before a shot fizzles
BULLET_MAX = 4
FIRE_COOLDOWN = 0.18

# radius, points, how many chunks it splits into
ROCK_TIERS = {3: (52, 20), 2: (30, 50), 1: (16, 100)}
START_LIVES = 3


def wrap(v: pygame.Vector2) -> pygame.Vector2:
    """Everything in Asteroids lives on a torus: off one edge, in the other."""
    return pygame.Vector2(v.x % WIDTH, v.y % HEIGHT)
...
class Ship:
    ...
    def update(self, dt: float, keys: pygame.key.ScancodeWrapper) -> None:
        ...
        self.vel *= max(0.0, 1.0 - SHIP_DRAG * dt)
...
class Bullet:
    def __init__(self, ship: Ship) -> None:
        ...
        self.vel = ship.vel + ship.heading() * BULLET_SPEED
        self.age = 0.0
...
class Game:
    ...
    def update(self, dt: float) -> None:
        ...
        for bullet in list(self.bullets):
            for rock in list(self.rocks):
                if bullet.pos.distance_to(rock.pos) < rock.radius:
                    ...
                    self.rocks += rock.split(self.rng)
        ...
        if self.ship.alive and self.ship.safe_timer <= 0:
            for rock in list(self.rocks):
